...
The RDKit embed and the Molstar demo case-study page each carry their own CSP, scoped by path in `next.config.ts`'s `headers()` (matched on `lib/rdkit-route.ts`'s `RDKIT_EMBED_PATH` and `MOLSTAR_DEMO_SLUG`) — the same per-route scoping this site always used for those two, just declared statically now instead of computed per request in `proxy.ts`.
