...
export async function SiteHeader({ locale }: { locale: Locale }) {
  ...
  const navLinks = [
    ...
    { label: t('technicalWriting'), href: siteConfig.social.medium, external: true },
  ]
    ...
    <header className="relative z-50 border-b border-gray-200 dark:border-gray-800">
      <div className="mx-auto max-w-4xl px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden sm:flex items-center gap-6 text-sm font-medium text-gray-600 dark:text-gray-400">
          <Link href="/about" className="hover:text-gray-900 transition-colors dark:hover:text-gray-100">{t('about')}</Link>
          <Link href="/work" className="hover:text-gray-900 transition-colors dark:hover:text-gray-100">{t('work')}</Link>
